...
class Evaluation:
    ...
    def _eval(self, output, target, maxval, mask=None, pixel_based=True, feature_based=True):
        score = {}
        use_mask = mask is not None and torch.any(mask)
        if use_mask:
            output = output * mask
            target = target * mask
        ...
        if pixel_based:
            ...
            ssim_score = float(ssim(target, output, maxval).item())
            psnr_score = float(psnr(target, output, maxval).item())
            if not np.isfinite(ssim_score):
                ssim_score = 1.0 if np.allclose(output, target) else 0.0
            if not np.isfinite(psnr_score):
                psnr_score = 99.0 if np.allclose(output, target) else 0.0
    ...
    def _get_normalization_params(self, output, target, mask=None):
        use_mask = mask is not None and torch.any(mask)
        if use_mask: # not usable for multibatch eval
        ...
        so_safe = torch.where(torch.abs(so) < 1e-8, torch.ones_like(so), so)
        scale = st / so_safe
        shift = mt - mo * scale
        return scale, shift
...
def classic_eval(dataset, model, forward_fn, data_transform, num_workers=4):
    ...
    for sample in tqdm(dataloader):
        target, maxval = sample.target, float(sample.max_value.item())
        target = target.unsqueeze(-3).to(device)
        if not np.isfinite(maxval) or maxval <= 0:
            maxval = max(float(torch.max(target).item()), 1e-8)
        with torch.no_grad():
        ...
        eval_mask = output_mask if torch.any(output_mask) else None
        eval.push(output, target, maxval, mask=eval_mask)
...
def pathology_eval(dataset, model, forward_fn, data_transform, num_workers=4):
    ...
    with tqdm(total=len(dataloader)) as pbar:
        for sample in dataloader:
            target, maxval, fname, slice_num = sample.target, float(sample.max_value.item()), sample.fname[0], sample.slice_num.item()
            target = target.unsqueeze(-3).to(device)
            if not np.isfinite(maxval) or maxval <= 0:
                maxval = max(float(torch.max(target).item()), 1e-8)
            with torch.no_grad():
            ...
            eval.push(output, target, maxval, mask=eval_mask, feature_based=True, pixel_based=False)

            # Get normalization parameters based on entire image
            scale, shift = eval._get_normalization_params(output, target, mask=eval_mask)
