...
class SliceDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, i: int):
        ...
        with h5py.File(fname, "r") as hf:
            ...
            # Prefer MVUE target when available, but gracefully fall back to
            # standard fastMRI recon targets for datasets that do not store MVUE.
            target_key = None
            max_key = None
            if "reconstruction_mvue" in hf:
                target_key = "reconstruction_mvue"
                max_key = "max_mvue"
            elif self.recons_key in hf:
                target_key = self.recons_key
                max_key = "max"
                ...
                raise KeyError(
                    f"Could not find a reconstruction target in {fname}. "
                    f"Expected one of: reconstruction_mvue, {self.recons_key}, reconstruction_rss. "
                    f"Available keys: {list(hf.keys())}"
                )

            target = hf[target_key][dataslice]
            ...
            # Use dataset-provided max if present; otherwise infer from target.
            max_val = hf.attrs[max_key] if (max_key is not None and max_key in hf.attrs) else None
            if max_val is None:
                max_val = hf.attrs["max"] if "max" in hf.attrs else float(np.max(np.abs(target)))
            max_val = float(max_val) if np.isfinite(max_val) else float(np.max(np.abs(target)))
            attrs['max'] = max(max_val, 1e-8)
...
class VarNetDataTransform:
    ...
    def __call__(
        ...
        sens_maps_arr = attrs.get("sensitivity_maps", None)
        if sens_maps_arr is not None:
            sens_maps = sens_maps_arr
            output_mask = np.abs(np.sum(sens_maps * np.conj(sens_maps), axis=0, keepdims=True))
            output_mask = output_mask > 0.5
            output_mask = transforms.to_tensor(output_mask).bool()
            sens_maps = transforms.to_tensor(sens_maps)
        else:
            output_mask = torch.ones_like(masked_kspace[:1, ..., 0], dtype=torch.bool)
...
class UnetDataTransform:
    ...
    def __call__(
        ...
        # Some datasets have target dims larger than masked image dims.
        # Clamp crop to image size to avoid invalid center-crop shapes.
        img_h, img_w = int(image.shape[-3]), int(image.shape[-2])
        ...
        # normalize target
        if target is not None:
            # Match target spatial size to model input crop.
            target = transforms.center_crop(target, crop_size)
            # Take absolute value of reconstruction target.
            target = np.abs(target)
            target_torch = transforms.to_tensor(target)
        else:
            target_torch = torch.Tensor([0])
        ...
        if sens_maps_arr is not None:
            ...
            output_mask = np.abs(np.sum(sens_maps * np.conj(sens_maps), axis=0, keepdims=True))
            output_mask = output_mask > 0.5
            output_mask = transforms.center_crop(output_mask, crop_size)
            output_mask = transforms.to_tensor(output_mask).bool()
        else:
            output_mask = torch.ones_like(image, dtype=torch.bool)
        
        
        return UnetSample(
            image=image.clone(),
            target=target_torch.clone(),
            fname=fname,
            slice_num=slice_num,
            max_value=max_value,
...
def get_dataloader(path_to_json, transform, num_workers=4, augment_data=False, batch_size=1, shuffle=True, rank=0, world_size=1, group_shape_by=None, load_sensitivity_maps=True):
    ...
    if group_shape_by is None:
        ...
        if isinstance(transform, UnetDataTransform) or transform is None:
            # For heterogeneous datasets, target shape can be larger than the
            # actual cropped image shape used by U-Net transforms. Grouping by
            # k-space + target avoids mixed-size batches.
            group_shape_by = 'kspace_and_target' if isinstance(transform, UnetDataTransform) else 'target'
...
def setup(mode, config, accl_factors, seed=0):
    ...
    # Default behavior is unchanged: map acceleration -> center fractions and use
    # equispaced masks, unless overridden by setup config.
    center_fractions, accelerations = mask_func_params(accl_factors)
    if config.get("center_fractions") is not None:
        center_fractions = list(config["center_fractions"])
    if config.get("accelerations") is not None:
        accelerations = list(config["accelerations"])

    mask_type = str(config.get("mask_type", "equispaced_fraction")).lower()
    if mask_type in ("equispaced", "equispaced_fraction"):
        mask_cls = subsample.EquiSpacedMaskFunc
    elif mask_type == "random":
        mask_cls = subsample.RandomMaskFunc
    else:
        raise ValueError(
            f"Unsupported mask_type '{mask_type}'. "
            "Expected one of: random, equispaced, equispaced_fraction."
        )
