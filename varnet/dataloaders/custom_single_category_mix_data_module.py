...
class SingleCategoryMixDataModule(pl.LightningDataModule):
    ...
    def _create_data_loader(self, data_transform: Callable, data_partition: str, sample_rate: Optional[float] = None, volume_sample_rate: Optional[float] = None) -> torch.utils.data.DataLoader:
        ...
        if is_train and self.combine_train_val:
            ...
            ds2_train = self._build_dataset2(self.data_path2 / f"{self.challenge}_train", data_transform, 0.5, 0.5, raw_sample_filter) # fixed 50% sampling for FastMRI
            ds2_val = self._build_dataset2(self.data_path2 / f"{self.challenge}_val", data_transform, 0.5, 0.5, raw_sample_filter) # fixed 50% sampling for FastMRI
            ...
            ds2 = self._build_dataset2(root2, data_transform, sample_rate, 0.3, raw_sample_filter)
